...
def solve(eqs, u, slns=None, subs=None, show=False, solver='default'):
    ...
    if slns == None :
        slns = [[0.] for i in range(len(u))]
    A = [[0. for j in range(len(u))] for i in range(len(u))]
    B = slns
    # A * u = B
    ...
    if solver == 'default' :
        rtn = np.linalg.solve(np.array(A),np.array(B))
    elif solver == 'gaussElim' :
        rtn = gaussElim(A,B)
    elif solver == 'triangluarReduction' :
        rtn = triangluarReduction(A,B)
...
def triangluarReduction(A,B):
    ...
    for i in range(0,n-1):
        ...
        for j in range(i+1,n):
            ...
            A[j] = [A[j][i] - A[i][i]*multiplier for i in range(n)]
